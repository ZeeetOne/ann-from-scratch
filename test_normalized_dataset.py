"""
Test multiclass dengan normalized dataset
"""
import requests
import json
...
def test_normalized_multiclass():
    print("=" * 70)
    print("TEST: Multiclass dengan Normalized Dataset")
    print("=" * 70)

    # Load example
    print("\n1. Loading multiclass example...")
    ...
    if result_before['success']:
        ...
        if 'results' in result_before and len(result_before['results']) > 0:
            print("\n   First 3 predictions:")
            for i in range(min(3, len(result_before['results']))):
                res = result_before['results'][i]
                if 'y_pred' in res:
                    print(f"   Sample {i}: y_pred={[f'{p:.3f}' for p in res['y_pred']]}")
                elif 'prediction' in res:
                    print(f"   Sample {i}: prediction={res['prediction']}")
    else:
    ...
    if result_train['success']:
        ...
        for i in range(min(5, len(result_train['predictions']))):
            pred = result_train['predictions'][i]
            y_true = pred['y_true']
            y_pred = pred['y_pred']
            y_class = pred['y_pred_classes']
            correct = "[OK]" if y_class == y_true else "[FAIL]"
            print(f"   Sample {i}: y_true={y_true}, y_pred={[f'{p:.3f}' for p in y_pred]}, class={y_class} {correct}")
        ...
        for pred in result_train['predictions']:
            for p in pred['y_pred']:
                if abs(p - 0.5) > 0.1:  # At least 0.1 away from 0.5
                    diverse = True
                    break
